    ...
    <div className="relative flex flex-col items-center">
      ...
      >
        ...
        >
          ...
          <div className="absolute inset-0 rounded-full bg-gradient-to-br from-[#DC2626] to-[#B91C1C] shadow-[0_4px_24px_rgba(220,38,38,0.35)]" />
          ...
          {isListening && (
            <div 
              ...
              style={{
                borderColor: `rgba(220, 38, 38, ${Math.max(0.4, voiceActivity)})`,
                boxShadow: `0 0 ${30 + voiceActivity * 30}px rgba(220, 38, 38, ${Math.max(0.25, voiceActivity)})`,
              }}
            />
          )}
          ...
          {turnState === 'ai' && !isProcessing && (
            <div
              className="absolute inset-0 rounded-full border-2 transition-all duration-200"
              style={{
                borderColor: `rgba(234, 179, 8, ${Math.max(0.4, aiPulse)})`,
                boxShadow: `0 0 ${30 + aiPulse * 30}px rgba(234, 179, 8, ${Math.max(0.25, aiPulse)})`,
              }}
            />
          )}
          ...
          {/* Processing Ring */}
          {isProcessing && (
            <div className="absolute inset-0 rounded-full border-2 border-white/40 animate-spin opacity-70" 
          ...
          <div className="absolute inset-0 rounded-full overflow-hidden">
            <div 
              ...
              `}
              style={{
                background: `radial-gradient(circle, ${glowMode === 'user' ? 'rgba(220, 38, 38, 0.4)' : 'rgba(234, 179, 8, 0.4)'} 0%, transparent 70%)`,
              }}
            />
          </div>
        </button>
      ...
      {isListening && (
        <div className="absolute top-full mt-2 sm:mt-3 flex items-center space-x-1.5 sm:space-x-2 text-[#DC2626]">
      ...
      {turnState === 'ai' && (
        <div className="absolute top-full mt-2 sm:mt-3 flex items-center space-x-1.5 sm:space-x-2 text-[#EAB308]">
