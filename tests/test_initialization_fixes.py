...
class TestAuthenticationFlow:
    ...
    async def test_user_registration_success(self, mock_db_manager):
        ...
        with patch('backend.database.db_manager.create_client') as mock_create:
            ...
            db_manager = DatabaseManager()
            result = await db_manager.register_user("test@example.com", "password123", "Test User")
...
class TestIntegrationWithMockAuth:
    ...
    @pytest.mark.asyncio
    async def test_mock_auth_initialization(self):
        ...
        with patch.dict(os.environ, {"USE_MOCK_AUTH": "true"}):
            ...
            with patch('backend.services.MockDatabaseManager') as mock_class:
                mock_instance = Mock()
